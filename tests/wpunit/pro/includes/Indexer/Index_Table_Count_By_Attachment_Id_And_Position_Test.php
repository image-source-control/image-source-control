 ...
 */
class Index_Table_Count_By_Attachment_Id_And_Position_Test extends WPTestCase {
	...
	public function test_count_with_invalid_attachment_id_returns_zero() {
		$count = $this->index_table->count_by_attachment_id_and_position( 0, 'content' );
	...
	public function test_count_with_invalid_position_returns_zero() {
		$this->index_table->insert_or_update( $this->post_id_1, $this->attachment_id, 'content' );

		$count = $this->index_table->count_by_attachment_id_and_position( $this->attachment_id, 'invalid_position' );
	...
	public function test_count_returns_zero_when_no_entries_exist() {
		$count = $this->index_table->count_by_attachment_id_and_position( $this->attachment_id, 'content' );
	...
	public function test_count_entries_for_specific_position() {
		...
		$this->index_table->insert_or_update( $this->post_id_1, $this->attachment_id, 'head' );
		$this->index_table->insert_or_update( $this->post_id_2, $this->attachment_id, 'head' );
		$this->index_table->insert_or_update( $this->post_id_3, $this->attachment_id, 'head' );

		$count = $this->index_table->count_by_attachment_id_and_position( $this->attachment_id, 'head' );
	...
	public function test_count_only_counts_specified_position() {
		...
		$this->index_table->insert_or_update( $this->post_id_1, $this->attachment_id, 'head' );
		$this->index_table->insert_or_update( $this->post_id_2, $this->attachment_id, 'body' );
		$this->index_table->insert_or_update( $this->post_id_3, $this->attachment_id, 'content' );

		$count_head = $this->index_table->count_by_attachment_id_and_position( $this->attachment_id, 'head' );
		$count_body = $this->index_table->count_by_attachment_id_and_position( $this->attachment_id, 'body' );
		$count_content = $this->index_table->count_by_attachment_id_and_position( $this->attachment_id, 'content' );
	...
	public function test_count_with_timestamp_filter() {
		...
		$this->index_table->insert_or_update(
			...
			$this->attachment_id,
			'body',
			$this->index_table->get_last_checked( $very_recent_timestamp )
		);

		// Count all entries (no timestamp filter)
		$count_all = $this->index_table->count_by_attachment_id_and_position( $this->attachment_id, 'body' );
		...
		$count_recent = $this->index_table->count_by_attachment_id_and_position( $this->attachment_id, 'body', $one_hour_ago );
		...
		$count_very_recent = $this->index_table->count_by_attachment_id_and_position( $this->attachment_id, 'body', $ten_minutes_ago );
	...
	public function test_count_with_future_timestamp_returns_zero() {
		...
		$count = $this->index_table->count_by_attachment_id_and_position( $this->attachment_id, 'head', $future_timestamp );
	...
	public function test_count_only_counts_specified_attachment() {
		...
		$this->index_table->insert_or_update( $this->post_id_1, $this->attachment_id, 'body' );
		$this->index_table->insert_or_update( $this->post_id_2, $this->attachment_id, 'body' );

		// Add entries for second attachment
		$this->index_table->insert_or_update( $this->post_id_3, $attachment_id_2, 'body' );

		$count_first = $this->index_table->count_by_attachment_id_and_position( $this->attachment_id, 'body' );
		$count_second = $this->index_table->count_by_attachment_id_and_position( $attachment_id_2, 'body' );
	...
	public function test_count_at_threshold_boundary() {
		...
		for ( $i = 1; $i <= $threshold; $i++ ) {
			...
			$this->index_table->insert_or_update( $post_id, $this->attachment_id, 'head' );
		}
