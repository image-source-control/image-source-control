 ...
 */
class Index_Table_Delete_Not_Updated_Since_Test extends WPTestCase {
	...
	public function test_delete_not_updated_since() {
		...
		$this->index_table->insert_or_update(
			...
			$attachment_id_3,
			'thumbnail',
			$this->index_table->get_last_checked( $current_time - 3720 )
		);
		...
		$this->index_table->insert_or_update(
			...
			$attachment_id_4,
			'content',
			$this->index_table->get_last_checked( $current_time - 1800 )
		);
		...
		$this->index_table->insert_or_update(
			...
			$attachment_id_5,
			'head',
			$this->index_table->get_last_checked( $current_time - 300 )
		);

		// Verify all 5 entries exist before deletion
		$count_before = $this->index_table->count_occurrences_by_post_id( $this->post_id );
		...
		$deleted = $this->index_table->delete_not_updated_since( $current_time - 3600 );

		// Should delete entries 1, 2, and 3 (3 entries total)
		$this->assertEquals( 3, $deleted, 'Should delete exactly 3 old entries' );

		// Verify remaining entries
		$remaining = $this->index_table->get_by_post_id( $this->post_id );
	...
	public function test_delete_not_updated_since_across_multiple_posts() {
		...
		$count_post_1 = $this->index_table->count_occurrences_by_post_id( $this->post_id );
		$count_post_2 = $this->index_table->count_occurrences_by_post_id( $post_id_2 );
		...
		$remaining_post_1 = $this->index_table->get_by_post_id( $this->post_id );
		...
		$remaining_post_2 = $this->index_table->get_by_post_id( $post_id_2 );
	...
	public function test_delete_not_updated_since_with_zero_timestamp() {
		...
		$deleted = $this->index_table->delete_not_updated_since( 0 );
	...
	public function test_delete_not_updated_since_with_negative_timestamp() {
		...
		$deleted = $this->index_table->delete_not_updated_since( - 100 );
	...
	public function test_delete_not_updated_since_applies_safety_margin() {
		...
		$this->index_table->insert_or_update(
			...
			$attachment_id_2,
			'content',
			$this->index_table->get_last_checked( $current_time - 3630 )
		);
	...
	public function test_delete_not_updated_since_with_empty_table() {
		$deleted = $this->index_table->delete_not_updated_since( time() - 3600 );
