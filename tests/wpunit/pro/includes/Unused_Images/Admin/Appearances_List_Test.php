...
class Appearances_List_Test extends WPTestCase {
	...
	public function test_render_without_results() {
		...
		$this->assertEquals( '–– unchecked -–', $output );
	}
	...
	public function test_render_shows_unused_when_checked_but_no_results() {
		...
		$this->assertEquals('–– unused -–', $output);
	}
	 ...
	 * - Post with Image 1
	 * - Post with Image 2
	 * <h4>Images found in post content</h4>
	 * - Post with Image 1
	 * - Post with Image 2
	 * <h4>Images found in the database</h4>
	 * –– unchecked -–
	 */
	public function test_render_with_indexer_results_in_content() {
		...
		$this->assertStringContainsString('<h4>Images found in post content</h4>', $output);

		// Should NOT show "no results" for indexer section
		$indexer_section_start = strpos($output, 'Images found in post content');
		$database_section_start = strpos($output, 'Images found in the database');
		$indexer_section = substr($output, $indexer_section_start, $database_section_start - $indexer_section_start);

		$this->assertStringNotContainsString('–– no results -–', $indexer_section);

		// The other sections are empty
		$this->assertStringContainsString('<h4>Images found in the database</h4>–– unchecked -–</details>', $output);
	...
	public function test_render_with_only_database_results() {
		...
		$this->assertStringContainsString('<h4>Images found in the database</h4>', $output);
		...
		if ($database_section_start !== false) {
			...
			if ($indexer_section_start !== false) {
				...
				$this->assertStringNotContainsString('–– no results -–', $database_section);
			}
		} else {
		...
		// The other sections are empty
		$this->assertStringContainsString('<h4>Images found in post content</h4>–– no results -–<h4>', $output);
	}
	 ...
	 *    <h4>Images found in the database</h4>
	 *      Database Post (Post)
	 *   <h4>Related posts from the Image Sources Index</h4>
	 *      Image Source Post (Post)
	 */
	public function test_render_with_different_post_in_all_data_sources() {
		...
		$this->assertStringContainsString('<h4>Images found in post content</h4>', $output);
		$this->assertStringContainsString('<h4>Images found in the database</h4>', $output);
		$this->assertStringContainsString('<h4>Related posts from the Image Sources Index</h4>', $output);

		// no unchecked or no-results
		$this->assertStringNotContainsString('–– unchecked -–', $output);
		$this->assertStringNotContainsString('–– no results -–', $output);
	 ...
	 * 3. Images found in post content
	 * 4. Images found in the database
	...
	public function test_render_output_structure_order() {
		...
		$indexer_pos = strpos( $output, 'Images found in post content' );
		$database_pos = strpos( $output, 'Images found in the database' );
