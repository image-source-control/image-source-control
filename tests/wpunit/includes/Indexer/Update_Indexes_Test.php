...
class Indexer_Update_Indexes_Test extends WPTestCase {
	...
	public function tearDown(): void {
		...
		if ( isset( $post->ID ) ) {
			delete_post_meta( $post->ID, Post_Meta::POST_IMAGE_META_KEY );
			delete_post_meta( $post->ID, Post_Meta::BEFORE_UPDATE_META_KEY );
		}
		...
		foreach ( $attachments as $att_id ) {
			delete_post_meta( $att_id, Post_Meta::IMAGE_POST_META_KEY );
		...
		foreach ( $posts as $post_id ) {
			delete_post_meta( $post_id, Post_Meta::POST_IMAGE_META_KEY );
			delete_post_meta( $post_id, Post_Meta::BEFORE_UPDATE_META_KEY );
		}
	...
	public function test_update_indexes_does_not_run_when_cannot_index_page() {
		...
		$this->assertEmpty( get_post_meta( $post->ID, Post_Meta::POST_IMAGE_META_KEY, true ), 'Meta should not be updated when cannot index page.' );
		$this->assertEmpty( get_post_meta( $post->ID, Post_Meta::BEFORE_UPDATE_META_KEY, true ), 'Temporary meta should not be present when cannot index page.' );
	}
	...
	public function test_update_indexes_skips_when_content_has_isc_list_all_shortcode() {
		...
		update_post_meta( $post->ID, Post_Meta::BEFORE_UPDATE_META_KEY, [ 1 => 'data' ] );
		...
		$this->assertSame( [], get_post_meta( $post->ID, Post_Meta::POST_IMAGE_META_KEY, true ), 'Indexer should set meta to empty array for isc_list_all shortcode.' );
		$this->assertEmpty( get_post_meta( $post->ID, Post_Meta::BEFORE_UPDATE_META_KEY, true ), 'Temporary meta should be cleaned up for isc_list_all shortcode.' );
	}
	...
	public function test_update_indexes_skips_when_attachments_already_set_and_not_bot() {
		...
		update_post_meta( $post->ID, Post_Meta::POST_IMAGE_META_KEY, [ 1 => 'data' ] );
		// Simulate temporary meta exists from a previous save
		update_post_meta( $post->ID, Post_Meta::BEFORE_UPDATE_META_KEY, [ 2 => 'old_data' ] );
		...
		$this->assertSame( [ 1 => 'data' ], get_post_meta( $post->ID, Post_Meta::POST_IMAGE_META_KEY, true ), 'Main meta should remain unchanged when skipping.' );
		$this->assertEmpty( get_post_meta( $post->ID, Post_Meta::BEFORE_UPDATE_META_KEY, true ), 'Temporary meta should be cleaned up when skipping.' );
	}
	...
	public function test_update_indexes_processes_content_on_first_run() {
		...
		$isc_post_images = get_post_meta( $post->ID, Post_Meta::POST_IMAGE_META_KEY, true );
		...
		$isc_image_posts1 = get_post_meta( $att1['id'], Post_Meta::IMAGE_POST_META_KEY, true );
		$isc_image_posts2 = get_post_meta( $att2['id'], Post_Meta::IMAGE_POST_META_KEY, true );
		...
		$this->assertEmpty( get_post_meta( $post->ID, Post_Meta::BEFORE_UPDATE_META_KEY, true ), 'Temporary meta should not be present after first run.' );
	}
	...
	public function test_update_indexes_removes_association_when_image_removed() {
		...
		update_post_meta( $post->ID, Post_Meta::BEFORE_UPDATE_META_KEY, $old_state );
		delete_post_meta( $post->ID, Post_Meta::POST_IMAGE_META_KEY ); // Ensure main index is missing

		// Simulate image->post meta exists for both images, including some dummy IDs (999, 888)
		update_post_meta( $att1['id'], Post_Meta::IMAGE_POST_META_KEY, [ $post->ID, 999 ] );
		update_post_meta( $att2['id'], Post_Meta::IMAGE_POST_META_KEY, [ 888, $post->ID ] );
		...
		$this->assertEmpty( get_post_meta( $post->ID, Post_Meta::BEFORE_UPDATE_META_KEY, true ), 'Temporary meta should be cleaned up.' );
	}
	...
	public function test_update_indexes_adds_association_when_image_added() {
		...
		update_post_meta( $att1['id'], Post_Meta::IMAGE_POST_META_KEY, [ $post->ID ] );
		update_post_meta( $att2['id'], Post_Meta::IMAGE_POST_META_KEY, [ 777 ] ); // Meta exists, but not for this post
	...
	public function test_update_indexes_handles_content_starting_empty() {
		...
		update_post_meta( $att1['id'], Post_Meta::IMAGE_POST_META_KEY, [ 999 ] );
	...
	public function test_update_indexes_runs_when_bot_is_running_and_meta_exists() {
		...
		// Simulate main meta exists
		update_post_meta( $post->ID, Post_Meta::POST_IMAGE_META_KEY, [ 999 => 'old_data' ] );
		// Simulate temporary meta exists from a previous save
		update_post_meta( $post->ID, Post_Meta::BEFORE_UPDATE_META_KEY, [ 999 => 'old_data' ] );
		// Simulate image meta exists for old image
		update_post_meta( 999, Post_Meta::IMAGE_POST_META_KEY, [ $post->ID ] );
		...
		$isc_image_posts_old = get_post_meta( 999, Post_Meta::IMAGE_POST_META_KEY, true );
		$isc_image_posts_new = get_post_meta( $att1['id'], Post_Meta::IMAGE_POST_META_KEY, true );
	...
	public function test_update_indexes_applies_isc_images_in_posts_simple_filter() {
		...
		$isc_image_posts_added = get_post_meta( $img_added_by_filter['id'], Post_Meta::IMAGE_POST_META_KEY, true );
		...
		$isc_image_posts_original = get_post_meta( $img_in_content['id'], Post_Meta::IMAGE_POST_META_KEY, true );
