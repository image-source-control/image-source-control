...
class Thumbnail_Indexing_Test extends WPTestCase {
	...
	public function test_first_index_with_thumbnail_only() {
		...
		$isc_image_posts = get_post_meta( $thumb['id'], Post_Meta::IMAGE_POST_META_KEY, true );
	...
	public function test_update_adds_thumbnail() {
		...
		$initial_post_images = get_post_meta( $post->ID, Post_Meta::POST_IMAGE_META_KEY, true );
		$this->assertArrayHasKey( $img1['id'], $initial_post_images );
		$this->assertFalse( $initial_post_images[ $img1['id'] ]['thumbnail'] ?? false );
		// --- End Initial Run ---

		// --- Prepare for Update ---
		Post_Meta::prepare_for_reindex( $post->ID ); // Move index to _before_update
		...
		// Assert thumbnail's index contains post
		$isc_image_posts_thumb = get_post_meta( $thumb['id'], Post_Meta::IMAGE_POST_META_KEY, true );
		...
		$isc_image_posts_img1 = get_post_meta( $img1['id'], Post_Meta::IMAGE_POST_META_KEY, true );
	...
	public function test_update_removes_thumbnail() {
		...
		// --- Prepare for Update ---
		Post_Meta::prepare_for_reindex( $post->ID );
	...
	public function test_update_changes_thumbnail() {
		...
		$isc_image_posts_thumb_b = get_post_meta( $thumb_b['id'], Post_Meta::IMAGE_POST_META_KEY, true );
		...
		// Assert removed thumbnail's index no longer contains the post ID.
		$isc_image_posts_thumb = get_post_meta( $thumb_a['id'], Post_Meta::IMAGE_POST_META_KEY, true );
		$this->assertIsArray( $isc_image_posts_thumb );
		$this->assertNotContains( $post->ID, $isc_image_posts_thumb, 'Removed thumbnail index should NOT contain post ID anymore.' );
	}
