...
class Isc_Gutenberg {
	...
	public function save_meta() {
		...
		$isc_fields = array( 'isc_image_source', 'isc_image_source_url', 'isc_image_source_own', 'isc_image_licence' );
		if ( isset( $_post['nonce'] ) && false !== wp_verify_nonce( $_post['nonce'], 'isc-gutenberg-nonce' ) ) {
			// CHeck if the user can edit the image and that the `key` is actually and ISC postmeta key.
