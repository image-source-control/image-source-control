...
use ISC_Log, ISC_Model, ISC_Class;
...
class Indexer {
	...
	public static function update_indexes( string $content ) {
		...
		// Skip any source output or indexing if this is a page with a Global list.
	...
	public static function can_index_the_page(): bool {
		...
		if ( isset( $_SERVER['REQUEST_URI'] ) ) {
			// phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized, WordPress.Security.ValidatedSanitizedInput.MissingUnslash
			ISC_Log::log( 'can index content for ' . $_SERVER['REQUEST_URI'] . ' and post ID ' . $post->ID );
		}
