...
class Caption extends Renderer {
	...
	public static function get( int $image_id, array $data = [], array $args = [] ) {
		...
		// add the pre-text
		$source = self::add_pretext( $source );
	...
	public static function add_style( string $source, int $image_id ) {
		...
		return apply_filters( 'isc_overlay_html_source', '<span class="isc-source-text">' . $source . '</span>', $image_id );
	}
	 ...
	 * @return string
	 */
	public static function add_pretext( $source ) {
		$options = self::get_options();
