...
$('#isc-signup-loader').addClass('hidden')}})});$('#isc_settings_section_signup .postbox-header .dashicons-no-alt').on('click',function(){$('#isc-signup-nl').prop('disabled',!0);$('#isc-signup-loader').removeClass('hidden');$.ajax({type:'POST',url:ajaxurl,data:{action:'newsletter_close',nonce:isc.ajaxNonce,},dataType:'json',success:function(response){$('#isc_settings_section_signup').remove()}})});document.querySelectorAll('#source-pretext-buttons button').forEach(function(button){button.addEventListener('click',function(){var inputField=document.getElementById('source-pretext');inputField.value+=this.textContent;inputField.focus()})});document.getElementById('source-pretext').addEventListener('focus',function(){document.getElementById('source-pretext-buttons').classList.remove('hidden')})});function isc_toggle_module_sections(){Object.entries(isc.moduleSections).forEach(([module,sections])=>{const checkbox=document.querySelector(`#isc-settings-plugin-modules input[value="${module}"]`);if(checkbox){sections.forEach(sectionId=>{const sectionElement=document.getElementById(sectionId);const section=sectionElement?sectionElement.querySelector('.inside'):null;if(section){if(checkbox.checked){section.classList.remove('hidden')}else{section.classList.add('hidden')}}})}})}
function isc_thumbnail_input_checkstate(){if(document.getElementById('use-thumbnail').checked){document.getElementById('thumbnail-size-select').classList.remove('hidden')}else{document.getElementById('thumbnail-size-select').classList.add('hidden')}
if('custom'===document.getElementById('thumbnail-size-select').value&&document.getElementById('use-thumbnail').checked){document.getElementById('isc-settings-custom-size').classList.remove('hidden')}else{document.getElementById('isc-settings-custom-size').classList.add('hidden')}}
function isc_caption_checkstate(){var overlay_enabled=document.getElementById('isc-settings-overlay-enable');if(!overlay_enabled){return}
var elements=document.querySelectorAll('.isc_settings_section_overlay .form-table tr:not(:first-of-type)');if(overlay_enabled.checked){Array.prototype.forEach.call(elements,function(el,i){el.style.display='table-row'})}else{Array.prototype.forEach.call(elements,function(el,i){el.style.display='none'})}}
function isc_licenses_checkstate(){var licenses_enabled=document.getElementById('isc-settings-licenses-enable');if(!licenses_enabled){return}
var elements=document.querySelectorAll('.isc_settings_section_licenses .form-table tr:not(:first-of-type)');if(licenses_enabled.checked){Array.prototype.forEach.call(elements,function(el,i){el.style.display='table-row'})}else{Array.prototype.forEach.call(elements,function(el,i){el.style.display='none'})}}
function isc_toggle_standard_source_text(){var standard_source_custom=document.getElementById('isc-custom-text-select');if(!standard_source_custom){return}
if(standard_source_custom.checked){document.getElementById('isc-custom-text').removeAttribute('disabled')}else{document.getElementById('isc-custom-text').setAttribute('disabled','disabled')}}
function isc_toggle_caption_position(){var caption_style=document.querySelector('#isc-settings-caption-style input:checked');if(!caption_style){return}
if(caption_style.value==='none'){document.getElementById('isc-settings-caption-position-options-wrapper').classList.add('hidden');document.getElementById('isc-settings-caption-position-options-wrapper').previousElementSibling.classList.add('hidden')}else{document.getElementById('isc-settings-caption-position-options-wrapper').classList.remove('hidden');document.getElementById('isc-settings-caption-position-options-wrapper').previousElementSibling.classList.remove('hidden')}}
function isc_show_reindex_warning(){document.getElementById('isc-settings-global-list-indexed-images-warning').classList.remove('hidden')}